...
async function obterContextoDaFracao(email) {
  try {
    ...
    if (error) {
      console.error("Erro ao obter contexto da fra��o:", error);
      return null;
    }
...
// -----------------------------
// 4. OPCAO B � Lan�amento pendente de comprovativos
// -----------------------------
async function lancarComprovativoPendente(aiData, contexto, comprovativoUrl) {
  try {
    ...
    console.error("Erro ao lan�ar comprovativo pendente:", e);
  }
}
...
export default async function handler(req, res) {
  try {
    ...
    if (!from || !subject) {
      console.error("Payload inv�lido (sem from ou subject):", req.body);
    ...
    const textoEmail =
      body && String(body).trim().length > 0
        ? body
        : "(sem texto � email cont�m apenas anexos ou conte�do n�o textual)";
    ...
    // 3. Obter contexto da fra��o
    const contexto = await obterContextoDaFracao(from);
    console.log("Contexto da fra��o:", contexto?.id_predio || null);
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Condom�nio <administracao@condomanagerai.com>",
          to: from,
    ...
    if (!aiData.subject || !aiData.message) {
      console.log("AI Router devolveu subject/message nulos ? s� autoresponder enviado.");
    ...
    // 8.1 � Ficheiro �nico (ex: recibo PDF)
    ...
    // 8.3 � Documentos autom�ticos da fra��o
    ...
    //
    // 9. OP��O B � Lan�amento pendente de comprovativos
    //
